...
class OAuthManager:
    ...
    def generate_oauth_uri(
        ...
        if not client_secret_file.exists():
            typer.echo(f"Error: Client secret file not found: {client_secret_file}", err=True)
            raise typer.Exit(1)
...
@app.command()
def setup(
    client_secret_file: Annotated[Path, typer.Argument(help="Path to OAuth client secret JSON file")],
    env_file: Annotated[Path, typer.Option("--env-file", "-e", help="Path to environment file")] = Path(".env"),
    scopes: Annotated[Optional[str], typer.Option("--scopes", "-s", help="Comma-separated OAuth scopes")] = None,
):
    """
    ...
    try:
        ...
        manager._update_env_var("OAUTH_TOKEN_URI", "https://oauth2.googleapis.com/token")
        
        typer.echo("\nOAuth configuration saved to environment file.")
        typer.echo("\nOAuth Authorization URI:")
        typer.echo(f"\n{auth_uri}\n")
        typer.echo("Please visit the above URL to complete the OAuth authorization flow.")
...
@app.command()
def create_auth(
    auth_id: Annotated[Optional[str], typer.Option("--auth-id", "-i", help="Authorization ID")] = None,
    env_file: Annotated[Path, typer.Option("--env-file", "-e", help="Path to environment file")] = Path(".env"),
):
    """
    Create OAuth authorization in Discovery Engine.
    ...
    token_uri = manager.env_vars.get("OAUTH_TOKEN_URI", "https://oauth2.googleapis.com/token")
    
    if not all([client_id, client_secret, auth_uri]):
        typer.echo("Error: OAuth credentials not found in environment. Run 'setup' command first.", err=True)
        raise typer.Exit(1)
    
    if manager.create_authorization(auth_id, client_id, client_secret, auth_uri, token_uri):
        typer.echo(f"\nAuthorization ID '{auth_id}' has been saved to environment file.")
        typer.echo("You can now link your agent to AgentSpace using this authorization.")
    else:
        raise typer.Exit(1)
...
def verify(
    ...
    if not auth_id:
        auth_id = manager.env_vars.get("OAUTH_AUTH_ID")
        if not auth_id:
            typer.echo("Error: No authorization ID provided or found in environment", err=True)
            raise typer.Exit(1)
...
def delete(
    ...
    if not force:
        confirm = typer.confirm(f"Are you sure you want to delete authorization '{auth_id}'?")
        if not confirm:
